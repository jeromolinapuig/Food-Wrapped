...
Scripts: `npm install`, `npm run dev`, `npm run build`, `npm run preview`. Configura `.env.local` con tus claves de Supabase (`VITE_SUPABASE_URL` y `VITE_SUPABASE_ANON_KEY`) y opcionalmente `VITE_HEART_AVATAR=true` para activar avatar con forma de corazón.
